...
use openssl::x509::{
    X509Builder, X509Extension, X509Name, X509NameBuilder, X509Ref, X509Req, X509ReqBuilder,
...
impl X509Cert {
    ...
    pub fn is_signed_by(&self, ca: &Self) -> Res<bool> {
        if ca.crt.issued(&self.crt) != X509VerifyResult::OK {
            Err("cert not issued by the ca")?;
        }
        Ok(self.crt.verify(&ca.key)?)
    }
    ...
    pub fn from_pkcs12_der<B: AsRef<[u8]>>(b: B, password: &str) -> Res<Self> {
        let pkcs12 = Pkcs12::from_der(b.as_ref())?.parse2(password)?;
        Ok(Self {
            key: pkcs12.pkey.ok_or("pkey missing")?,
            crt: pkcs12.cert.ok_or("x509 cert missing")?,
        })
    }

    pub fn key_and_crt_from_pem<B: AsRef<[u8]>>(key_pem: B, crt_pem: B) -> Res<Self> {
